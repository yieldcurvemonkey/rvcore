...
class GithubFetcher(BaseFetcher):
    ...
    def fetch_USD_MONKEY_CUBE_data(
        ...
        MONKEY_CUBE_PATH_DIR: Optional[str] = "yieldcurvemonkey/USD_SABR_VOL_CUBE/refs/heads/main",
    ...
    def fetch_USD_MONKEY_CUBE_cache(
        ...
        if missing_dates:
            fetch_results = self.fetch_USD_MONKEY_CUBE_data(
                ...
                MONKEY_CUBE_PATH_DIR=MONKEY_CUBE_PATH_DIR,
            )
    ...
    def fetch_USD_MONKEY_CUBE_ql_vol_cubes(
        ...
        sabr_params_ts_dict = self.fetch_USD_MONKEY_CUBE_sabr_params(
            dates=dates,
            show_tqdm=show_tqdm,
            max_concurrent_tasks=max_concurrent_tasks,
            max_keepalive_connections=max_keepalive_connections,
        ...
        ql_discount_curve_dict = {datetime(k.year, k.month, k.day): v for k, v in ql_discount_curve_dict.items()}
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(
                    ...
                    sabr_params_ts_dict[dt] if use_sabr and dt in sabr_params_ts_dict else None,
                    precalibrate_cube,
                ): dt
                for dt in vol_cube_ts_dict.keys()
            }
            if show_tqdm:
...
def build_usd_monkey_cube_ql_vol_handle(
    ...
    curr_monkey_cube_ql_on_index = ql.OvernightIndexedSwapIndex(
        "USD-SOFR-1D", ql.Period("1D"), 2, ql.USDCurrency(), ql.Sofr(ql.YieldTermStructureHandle(ql_discount_curve))
    )
