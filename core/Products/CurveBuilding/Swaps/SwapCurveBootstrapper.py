...
from core.Products.CurveBuilding.ql_curve_building_utils import build_piecewise_ql_discount_curve, build_ql_discount_curve, get_nodes_dict
...
class SwapCurveBootstrapper:
    ...
    def parallel_ql_swap_curve_bootstrapper(
        self,
        curve: str,
        ...
        for curve_date, discount_curve_nodes_dict in parallel_results.items():
            ...
            ql_curve = build_ql_discount_curve(
                datetime_series,
                type_series,
                CME_SWAP_CURVE_QL_PARAMS[curve]["dayCounter"],
                CME_SWAP_CURVE_QL_PARAMS[curve]["calendar"],
...
def bootstrap_single_swap_curve(
    ...
    try:
        ...
        CME_SWAP_CURVE_QL_PARAMS[curve]["dayCounter"]
        dummy_curve = ql.FlatForward(datetime_to_ql_date(as_of_date), 0.0, CME_SWAP_CURVE_QL_PARAMS[curve]["dayCounter"])
        dummy_handle = ql.YieldTermStructureHandle(dummy_curve)

        if CME_SWAP_CURVE_QL_PARAMS[curve]["is_ois"]:
            ql_floating_index = CME_SWAP_CURVE_QL_PARAMS[curve]["swapIndex"](dummy_handle)
            rate_helpers = [
            ...
            ql_floating_index = ql.IborIndex(
                ...
                CME_SWAP_CURVE_QL_PARAMS[curve]["calendar"],
                CME_SWAP_CURVE_QL_PARAMS[curve]["businessConvention"],
                False,
                dummy_handle,
            )
            rate_helpers = [
                ql.SwapRateHelper(
                    ...
                    ql.Period(term),
                    CME_SWAP_CURVE_QL_PARAMS[curve]["calendar"],
                    CME_SWAP_CURVE_QL_PARAMS[curve]["frequency"],
                    CME_SWAP_CURVE_QL_PARAMS[curve]["businessConvention"],
                    CME_SWAP_CURVE_QL_PARAMS[curve]["dayCounter"],
                    ql_floating_index,
                )
